...
class Encoder:
    ...
    def __init__(self, encoder_type="uni", num_layers=1,
    ...
    def build(self, input_vector, sequence_length, scope=None, reuse=False):
        with tf.variable_scope('build_uniencoder', reuse=reuse) as scope:
            if self.encoder_type == self.UNI_ENCODER_TYPE:
                '''
                initializer = tf.random_uniform_initializer(-1, 1)
                cell = tf.nn.rnn_cell.LSTMCell(self.num_units, initializer=initializer)
                outputs, _ = tf.nn.dynamic_rnn(cell, input_vector, sequence_length=sequence_length, dtype=tf.float32, time_major=False)
                return outputs, _
                '''
                ...
                raise ValueError("Unknown encoder_type {self.encoder_type}")
    ...
    def _rnn_single_cell(self):
        ...
        if self.cell_type == self.RNN_GRU_CELL:
            ...
            raise ValueError("Unknown rnn cell type. {self.cell_type}")
